...
import { Link } from 'react-router-dom'; // ✅ Enable routing to crop details
...
function SpringPage() {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:3000/crops/season/Spring')
      .then((res) => setCrops(res.data))
      .catch((err) => {
        console.error('Failed to fetch spring crops:', err);
        setError('Failed to load spring crops. Please try again later.');
      })
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-green-50 via-white to-green-100 min-h-screen">
      <div className="mb-6 max-w-xl mx-auto">
        <input
          type="text"
          placeholder="🔍 Search spring crops..."
      ...
      {loading ? (
        <div className="text-center text-lg text-green-600">Loading crops...</div>
      ) : error ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredCrops.map((crop, index) => (
            <Link
              to={`/crop/${crop._id}`} // ✅ Click to navigate to detail page
              key={index}
